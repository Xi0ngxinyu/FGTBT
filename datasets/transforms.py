...
def transform_pixel_float(pt, center, scale, output_size, invert=0, rot=0):
    # Transform pixel location to different reference
    # float
    t = get_transform(center, scale, output_size, rot=rot)
    if invert:
...
def generate_gaussVector(pt, sigma, label_type='Gaussian'):
    x0, y0 = pt
    vector = np.arange(0, 64, 1, np.float32)
    if label_type == 'Gaussian':
        g_x = np.exp(- ((vector - x0) ** 2) / (2 * sigma ** 2))
        g_y = np.exp(- ((vector - y0) ** 2) / (2 * sigma ** 2))
    else:
        g_x = sigma / (((vector - x0) ** 2 + sigma ** 2) ** 1.5)
        g_y = sigma / (((vector - y0) ** 2 + sigma ** 2) ** 1.5)
    return g_x, g_y

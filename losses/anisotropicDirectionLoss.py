...
class AnisotropicDirectionLoss(nn.Module):
    ...
    def _get_normals_from_neighbors(self, landmarks):
        ...
        normal_vector = torch.where(previous_tangent_vector == next_tangent_vector,
                                    self._inverse_vector(previous_tangent_vector), bi_normal_vector)
    ...
    def _get_loss_lambda(self, pv_gt, normal_force, tangent_force, normal_vector, tangent_vector, heatmap=None,
                         landmarks=None, lambda_mode=3):
        # fix
        if lambda_mode == 1:
            # 1
        ...
        # dynamic
            ...
            loss_lambda = torch.clamp(tangent_force.pow(2) / torch.clamp(normal_force.pow(2), min=1e-6), min=1.0,
                                      max=9.0)
            # b x n
        ...
        # heatmap-based
            ...
            normal_std2 = torch.mul(direction, normal_vector.unsqueeze(-1).unsqueeze(-1)).sum(dim=2, keepdim=False).pow(
                2) * weightmap  # b x n x h x w
            tangent_std2 = torch.mul(direction, tangent_vector.unsqueeze(-1).unsqueeze(-1)).sum(dim=2,
                                                                                                keepdim=False).pow(
                2) * weightmap  # b x n x h x w

            loss_lambda = torch.clamp(tangent_std2.sum([2, 3]) / torch.clamp(normal_std2.sum([2, 3]), min=1e-6),
                                      min=1.0, max=9.0)  # b x n
            # b x n
            loss_lambda = loss_lambda.detach()
        # statistic
        elif lambda_mode == 4:
        ...
        # statistic
            ...
            loss_lambda = torch.clamp(tangent_std2 / torch.clamp(normal_std2, min=1e-6), min=1.0, max=9.0).unsqueeze(
                0)  # 1 x n
            # 1 x n
            loss_lambda = loss_lambda.detach()
        else:
    ...
    def forward(self, output, groundtruth, heatmap=None, landmarks=None):
        ...
        """
        # heatmap = output
        # landmarks = groundtruth
        # output = get_preds(output[:, :68, :])
        # groundtruth = get_preds(groundtruth[:, :68, :])
        # 得到每个点所对应的法向量方向
        ...
        loss_lambda = self._get_loss_lambda(pv_gt.detach(), normal_force.detach(), tangent_force.detach(),
                                            normal_vector.detach(), tangent_vector.detach(), heatmap.detach(),
                                            landmarks.detach())

...
def create_logger(cfg):
    ...
    logger.setLevel(logging.INFO)  # 只记录info级别的日志
    ...
    log_file = cfg.OUTPUT_DIR + f"/logfile/log_{current_time}.log"

    # 创建一个文件处理器
    file_handler = logging.FileHandler(log_file)
    file_handler.setLevel(logging.INFO)  # 只记录info级别的日志

    # 创建一个控制台处理器（StreamHandler）
    console_handler = logging.StreamHandler()
    console_handler.setLevel(logging.INFO)  # 只记录info级别的日志

    # 创建一个格式器并添加到处理器
    # 设置为白色输出
    formatter = logging.Formatter('\033[97m%(asctime)s - %(levelname)s - %(message)s\033[0m')
    file_handler.setFormatter(formatter)  # 文件输出
    console_handler.setFormatter(formatter)  # 控制台输出
...
def calc_ber(y_pred, y_true):
    ...
    # 返回正类的平均错误率 负类的平均错误率 两个平均错误率的评均值
